...
1. **💰 便捷買賣輸入 (Sidebar)**
   ...
   * **手續費自動計算**：在填寫「單價」與「股數」時，會根據手續費率自動換算並填入手續費欄位。台股（市場為 `TPE`）會四捨五入至整數，其餘市場則保留小數點後兩位。
...
* **淨未實現損益對齊**：為與券商 APP 的「淨未實現損益」對齊，Dashboard 的未實現損益公式會自動扣除**預估賣出手續費與證券交易稅**（台股一般股票 0.3%，台股 ETF 0.1%，美股暫不計證交稅，手續費率採用您設定的全域手續費率）。這樣您在 `個股交易紀錄` 中不需手動加上任何預估稅費，即可讓兩邊的損益數字基本保持一致。
