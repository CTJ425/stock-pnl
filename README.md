...
1. **💰 便捷買賣輸入 (Sidebar)**
   ...
   * **張/零股單位切換**：台股股數可依「張」或「零股」輸入並自動換算（1 張 = 1000 股）；選擇美股市場時自動鎖定以「零股（股）」為單位。
...
3. **📅 年度收益總覽 (已實現損益)**
   ...
   * 自動以年度為單位彙整已賣出部位的已實現損益、買入/賣出總額、手續費與交易筆數；報表分為上下兩個獨立區塊——「🇹🇼 台股 (TWD)」與「🇺🇸 美股 (USD)」各自擁有完整的年度表格，兩個市場的數字完全分開、不混合幣別。合計以各年度列呈現，全量歷史累計數據見頂部 KPI 摘要卡。
   * 每個年度總計行可透過左側 `[+]`/`[-]` 展開或折疊該年度**有賣出交易**的個股明細（含代號、名稱與各項金額）。

4. **⚡ 自動同步與一鍵重掃**
   * 透過側邊欄點擊「確認送出」時，系統會自動在背景更新「庫存總覽 Dashboard」與「年度收益總覽」，無需手動點選功能選單。
   * **`🔄 重新掃描並更新所有報表`**：一鍵重新掃描「個股交易紀錄」，同時重建 Dashboard 與年度收益總覽——手動修改或刪除交易紀錄後，點一次即可讓所有報表回到正確數字。
...
* `code/code.gs`：核心邏輯，包含選單建立、側邊欄調用、計算引擎、搜尋模組與 Dashboard 寫入。
* `code/YearReport.gs`：年度收益總覽報表寫入邏輯。
...
* 點選自訂選單中的 **`💰 買賣輸入`**、**`📊 建立庫存總覽 Dashboard`**、**`📅 建立年度收益總覽`** 或 **`🔄 重新掃描並更新所有報表`**。
...
2. 將預設的 `代碼.gs`（或建立新指令碼檔案）重新命名為 **`code.gs`**，並將專案中 **`code/code.gs`** 的內容複製貼上。
3. 建立一個新指令碼檔案命名為 **`YearReport.gs`**，並將專案中 **`code/YearReport.gs`** 的內容複製貼上。
...
### 2. 手動更新與重建 / 刪除錯誤紀錄
* **手動修改資料**：若您手動修改了 `個股交易紀錄` 中的舊資料，請務必手動觸發重建以更新統計數據。推薦使用一鍵重掃：
  * **`🔄 重新掃描並更新所有報表`**（同時重建 Dashboard 與年度收益總覽）
  * 亦可個別執行 **`📊 建立庫存總覽 Dashboard`** 或 **`📅 建立年度收益總覽`**。
* **刪除錯誤紀錄**：若需要刪除輸入錯誤的交易紀錄，請直接前往 **`個股交易紀錄`** 頁籤中將該筆錯誤資料整列刪除。刪除後，點選選單中的 **`🔄 重新掃描並更新所有報表`** 一次重新計算並更新兩份報表。
...
* **淨未實現損益對齊與進位收整**：為與券商 APP 的「淨未實現損益」對齊，Dashboard 的未實現損益公式會自動扣除**預估賣出手續費與證券交易稅**（僅台股適用）。
  * **台股精算同構**：預估的賣出手續費與證交稅在公式中會**分別以 `FLOOR()` 無條件捨去至個位數**（與券商 App 及 Sidebar 的 `Math.floor` 邏輯同構），並且損益最外層套用 `ROUND(..., 0)`，以四捨五入收整移動平均成本因除權息或賣出產生的浮點尾數，確保與 App 損益金額一致。
  * **幣別分區塊格式化**：台股的市值與損益（Column F/G/H/I）顯示格式設定為**無小數的整數**（如 `NT$#,##0`），而現價與平均成本（Column C/E）則**保留兩位小數**（如 `NT$143.50`，因為台股價格本身有小數）；美股則全面保留兩位小數點並冠以 `US$`。
  * 若 `GOOGLEFINANCE` 暫時抓不到現價，市值與未實現損益會顯示為空白（而非誤顯示為全額虧損），現價恢復後自動回填。
